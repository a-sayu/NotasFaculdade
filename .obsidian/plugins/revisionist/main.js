...
var AIModelUtils = {
  ...
  getDefaultModelForProvider(provider) {
    ...
    return models == null ? void 0 : models[0];
  }
};
...
var SettingsService = class {
   ...
   */
  async updateLMStudioSettings(port, modelName) {
...
var SettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  addProviderSelection(containerEl) {
    const settings = this.settingsService.getSettings();
    new import_obsidian.Setting(containerEl).setName("AI provider").setDesc("Select which AI provider to use").addDropdown((dropdown) => {
      ...
      dropdown.setValue(settings.provider).onChange(async (value) => {
        await this.settingsService.updateSetting("provider", value);
        this.display();
      });
    });
  }
  addProviderSpecificSettings(containerEl) {
    const settings = this.settingsService.getSettings();
    if (settings.provider === "openrouter" /* OpenRouter */) {
      this.addOpenRouterSettings(containerEl);
    } else if (settings.provider === "lmstudio" /* LMStudio */) {
      this.addLMStudioSettings(containerEl);
    } else if (settings.provider === "openai" /* OpenAI */) {
  ...
  async handleTestConnection(button) {
    ...
    try {
      ...
      if (success) {
        new import_obsidian.Notice("Connection test successful!");
      } else {
        new import_obsidian.Notice("Connection test failed. Please check your settings.");
      }
    } catch (error) {
      new import_obsidian.Notice(`Connection test error: ${error.message}`);
    } finally {
      button.setButtonText("Test connection");
      button.setDisabled(false);
    }
  }
  addOpenRouterSettings(containerEl) {
    const settings = this.settingsService.getSettings();
    new import_obsidian.Setting(containerEl).setName("OpenRouter API key").setDesc("Enter your OpenRouter API key").addText((text) => {
      text.setPlaceholder("Enter API key").setValue(settings.apiKeys["openrouter" /* OpenRouter */]).onChange(async (value) => {
        await this.settingsService.setApiKey("openrouter" /* OpenRouter */, value);
        this.plugin.updateAdapterConfig();
      });
      text.inputEl.type = "password";
    }).addExtraButton((button) => {
      button.setIcon("external-link").setTooltip("Get API key").onClick(() => {
  ...
  addLMStudioSettings(containerEl) {
    const settings = this.settingsService.getSettings();
    new import_obsidian.Setting(containerEl).setName("LM Studio port").setDesc("Enter the port number for your local LM Studio instance").addText((text) => {
      text.setPlaceholder("1234").setValue(settings.lmStudio.port).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Model name").setDesc("Enter the name of your local model").addText((text) => {
      text.setPlaceholder("Model name").setValue(settings.lmStudio.modelName).onChange(async (value) => {
        await this.settingsService.updateLMStudioSettings(
  ...
  addDefaultSettings(containerEl) {
    const settings = this.settingsService.getSettings();
    if (settings.provider === "openrouter" /* OpenRouter */ || settings.provider === "openai" /* OpenAI */) {
      new import_obsidian.Setting(containerEl).setName("Default model").setDesc("Select the default model to use").addDropdown((dropdown) => {
        const models = AIModelMap[settings.provider];
        models.forEach((model) => {
          dropdown.addOption(model.apiName, model.name);
        });
        dropdown.setValue(settings.defaultModel).onChange(async (value) => {
          await this.settingsService.updateSetting("defaultModel", value);
        });
      });
    }
    new import_obsidian.Setting(containerEl).setName("Default temperature").setDesc("Set the default temperature for the AI model (0.0 - 1.0)").addSlider((slider) => {
      slider.setLimits(0, 1, 0.05).setValue(settings.defaultTemperature).setDynamicTooltip().onChange(async (value) => {
        await this.settingsService.updateSetting("defaultTemperature", value);
      });
    });
  }
  addOpenAISettings(containerEl) {
    const settings = this.settingsService.getSettings();
    new import_obsidian.Setting(containerEl).setName("OpenAI API key").setDesc("Enter your OpenAI API key").addText((text) => {
      text.setPlaceholder("Enter API key").setValue(settings.apiKeys["openai" /* OpenAI */]).onChange(async (value) => {
        await this.settingsService.setApiKey("openai" /* OpenAI */, value);
        this.plugin.updateAdapterConfig();
      });
      text.inputEl.type = "password";
    }).addExtraButton((button) => {
...
var CONFIG = {
  ...
  PROMPTS: {
     ...
     */
    formatUserPrompt: (instructions, selectedText, fullNote) => `
...
// src/ui/revisionModal.ts
var RevisionModal = class extends import_obsidian2.Modal {
  constructor(app, settingsService, selectedText, fullNoteContent, onSubmit) {
    super(app);
    this.settingsService = settingsService;
    this.selectedText = selectedText;
    this.fullNoteContent = fullNoteContent;
    ...
    this.result = {
      ...
      fullNoteContent: this.fullNoteContent
      // Store it in result
    };
  }
  onOpen() {
    ...
    CONFIG.SUGGESTION_PROMPTS.forEach((prompt) => {
      const buttonContainer2 = quickButtonsSection.createDiv({ cls: "quick-button-container" });
      const button = new import_obsidian2.ButtonComponent(buttonContainer2).setClass("quick-button").setClass("mod-cta").setButtonText(prompt.type.charAt(0).toUpperCase() + prompt.type.slice(1)).onClick(() => {
    ...
    new import_obsidian2.Setting(instructionsSection).setName("Revision instructions").setDesc("How would you like the text to be revised?").addTextArea((text) => {
      this.instructionsEl = text;
      text.setPlaceholder("Enter your instructions here...").setValue(this.result.instructions).onChange((value) => {
    ...
    const settings = this.settingsService.getSettings();
    if (settings.provider === "openrouter" /* OpenRouter */ || settings.provider === "openai" /* OpenAI */) {
      new import_obsidian2.Setting(contentEl).setName("AI model").setDesc("Select the model to use for revision").addDropdown((dropdown) => {
        this.modelDropdown = dropdown;
        const models = AIModelMap[settings.provider];
        models.forEach((model) => {
          dropdown.addOption(model.apiName, model.name);
        });
        dropdown.setValue(this.result.model).onChange((value) => {
          this.result.model = value;
        });
      });
    }
    new import_obsidian2.Setting(contentEl).setName("Temperature").setDesc("Higher values make the output more creative, lower values make it more consistent").addSlider((slider) => {
      slider.setLimits(0, 1, 0.05).setValue(this.result.temperature).setDynamicTooltip().onChange((value) => {
        this.result.temperature = value;
      });
    });
    const buttonContainer = contentEl.createDiv({ cls: "button-container" });
    new import_obsidian2.ButtonComponent(buttonContainer).setButtonText("Submit").setCta().onClick(() => {
  ...
  validateInput() {
    if (!this.result.instructions.trim()) {
      new import_obsidian2.Notice("Please enter revision instructions");
      return false;
    }
    return true;
  }
...
var ResultModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    this.revisedTextArea = new import_obsidian3.TextAreaComponent(revisedSection);
    this.revisedTextArea.setValue(this.props.revisedText).setPlaceholder("Revised text will appear here...");
  ...
  createButton(container, text, cls, onClick) {
  ...
  applyChanges() {
    ...
    new import_obsidian3.Notice("Text replaced successfully");
    this.close();
  }
...
var BaseAdapter = class {
  ...
  async generateResponse(prompt, modelApiName, options) {
    ...
    try {
      ...
      const temperature = (_a3 = options == null ? void 0 : options.temperature) != null ? _a3 : 0.7;
      const maxTokens = (_b = options == null ? void 0 : options.maxTokens) != null ? _b : 1e3;
      const response = await this.makeApiRequest({
        ...
        selectedText: (options == null ? void 0 : options.selectedText) || "",
        isTest: false
      });
  ...
  async testConnection() {
    ...
    try {
      ...
      const response = await this.makeApiRequest({
        model: ((_a3 = this.models[0]) == null ? void 0 : _a3.apiName) || "",
        prompt: "Hi",
        temperature: 0.7,
        maxTokens: 10,
        rawResponse: true,
      ...
      return response.status === 200 && !!((_e = (_d = (_c = (_b = response.json) == null ? void 0 : _b.choices) == null ? void 0 : _c[0]) == null ? void 0 : _d.message) == null ? void 0 : _e.content);
    } catch (error) {
      return false;
    }
  }
  ...
  handleError(error) {
    ...
    new import_obsidian4.Notice(`${provider} API Error: ${errorMessage}`);
  ...
  async validateApiKey() {
    try {
      ...
      if (isValid) {
        new import_obsidian4.Notice(`${this.getProviderType()} API key validated successfully`);
        return true;
      } else {
        throw new Error("Failed to validate API key");
      }
    } catch (error) {
      new import_obsidian4.Notice(`Failed to validate ${this.getProviderType()} API key: ${error instanceof Error ? error.message : "Unknown error occurred"}`);
      return false;
    }
  }
  /**
   ...
   */
  isReady() {
    const ready = (!!this.apiKey || this.getProviderType() === "lmstudio" /* LMStudio */) && this.models.length > 0;
    return ready;
  }
  /**
  ...
  getApiModelName(modelApiName) {
    ...
    if (!model) {
      return ((_a3 = this.models[0]) == null ? void 0 : _a3.apiName) || modelApiName;
    }
...
var OpenRouterAdapter = class extends BaseAdapter {
  ...
  async makeApiRequest(params) {
    try {
      const messages = params.isTest ? [{ role: "user", content: params.prompt }] : [
        {
          role: "system",
  ...
  extractContentFromResponse(response) {
    var _a3, _b, _c, _d;
    if (!((_d = (_c = (_b = (_a3 = response.json) == null ? void 0 : _a3.choices) == null ? void 0 : _b[0]) == null ? void 0 : _c.message) == null ? void 0 : _d.content)) {
      throw new Error("Invalid response format from OpenRouter API");
    }
    return response.json.choices[0].message.content;
  }
  extractTokenCounts(response) {
    var _a3;
    const usage = (_a3 = response.json) == null ? void 0 : _a3.usage;
    return {
      input: (usage == null ? void 0 : usage.prompt_tokens) || 0,
      output: (usage == null ? void 0 : usage.completion_tokens) || 0,
      total: (usage == null ? void 0 : usage.total_tokens) || 0
    };
  }
  getProviderType() {
...
var LMStudioAdapter = class extends BaseAdapter {
  constructor() {
    super(...arguments);
    this.port = "1234";
    // Default port
...
var OpenAIAdapter = class extends BaseAdapter {
  ...
  ensureClient() {
    if (!this.client && this.apiKey) {
      ...
      this.client = new OpenAI({
        ...
        // Allow usage in Obsidian's Electron environment
      });
    }
    if (!this.client) {
  ...
  extractTokenCounts(response) {
    ...
    return {
      input: (usage == null ? void 0 : usage.input_tokens) || 0,
      output: (usage == null ? void 0 : usage.output_tokens) || 0,
      total: (usage == null ? void 0 : usage.total_tokens) || 0
    };
  }
  /**
  ...
  async generateResponse(prompt, modelApiName, options) {
    ...
    try {
      ...
      const input = CONFIG.PROMPTS.formatUserPrompt(
        prompt,
        (options == null ? void 0 : options.selectedText) || "",
        (options == null ? void 0 : options.fullNote) || ""
      );
      ...
      const tokens = {
        input: ((_c = response.usage) == null ? void 0 : _c.input_tokens) || 0,
        output: ((_d = response.usage) == null ? void 0 : _d.output_tokens) || 0,
        total: ((_e = response.usage) == null ? void 0 : _e.total_tokens) || 0
      };
      return {
        success: true,
...
// src/main.ts
var AIRevisionPlugin = class extends import_obsidian7.Plugin {
  async onload() {
    ...
    this.addRibbonIcon("wand-2", "Revise selected text", () => {
      ...
      if (activeView == null ? void 0 : activeView.editor) {
        ...
        if (selectedText) {
          ...
          new import_obsidian7.Notice("Please select text to revise");
        }
      }
    });
    this.addCommand({
      ...
      this.app.workspace.on(
        ...
        (menu, editor) => {
          if (editor.getSelection()) {
            menu.addItem((item) => {
              item.setTitle("Revise with AI").setIcon("wand-2").onClick(() => {
                this.handleRevisionRequest(editor);
              });
            });
          }
    ...
    try {
      switch (settings.provider) {
        case "openrouter" /* OpenRouter */:
          ...
          if (!settings.apiKeys["openrouter" /* OpenRouter */]) {
          }
          this.aiAdapter.setApiKey(settings.apiKeys["openrouter" /* OpenRouter */]);
          break;
        ...
        case "openai" /* OpenAI */:
          ...
          if (!settings.apiKeys["openai" /* OpenAI */]) {
          }
          this.aiAdapter.setApiKey(settings.apiKeys["openai" /* OpenAI */]);
          break;
        default:
          return;
      }
    } catch (error) {
      new import_obsidian7.Notice("Failed to initialize AI provider. Check console for details.");
    }
  }
  /**
   ...
   */
  calculateApproximateCost(tokens, modelName) {
    const model = AIModelUtils.getModelByApiName(modelName);
    if (!model || !model.inputCostPer1M || !model.outputCostPer1M) {
      return void 0;
    }
    const inputCost = tokens.input / 1e6 * model.inputCostPer1M;
    const outputCost = tokens.output / 1e6 * model.outputCostPer1M;
    return {
      input: inputCost,
      output: outputCost,
   ...
   */
  async handleRevisionRequest(editor) {
    const selectedText = editor.getSelection();
    const fullNoteContent = editor.getValue();
    if (!selectedText) {
      new import_obsidian7.Notice("Please select text to revise");
      return;
    }
    ...
    if (wordCount > WORD_LIMIT) {
      new import_obsidian7.Notice(`Warning: Selected text is ${wordCount} words. The AI may struggle with more than ${WORD_LIMIT} words at once. Consider selecting a smaller portion.`, 1e4);
    }
    if (!this.aiAdapter.isReady()) {
      new import_obsidian7.Notice("AI provider is not properly configured. Please check settings.");
      return;
    }
    const revisionModal = new RevisionModal(
      ...
      selectedText,
      fullNoteContent,
      // Pass full note content
      async (result) => {
        try {
          new import_obsidian7.Notice("Generating revision...");
          ...
          const cost = response.tokens ? this.calculateApproximateCost(response.tokens, result.model) : void 0;
          new ResultModal(
            this.app,
            {
              ...
              cost
              // Simply pass the cost object without the isApproximate flag
            }
          ).open();
        } catch (error) {
